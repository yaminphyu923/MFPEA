...
@section('ml-general-stock-active', 'active')
            ...
            <div class="col-sm-12">
                <div class="card mb-3 card-button">
                    <div class="card-header d-flex justify-content-between">
                        <h4 class="mt-2"><b>Micro Lab General Stock List</small></b></h4>
                        <div class="justify-content-end">
                            <a href="{{ route('ml_general_stock_export') }}">
                            ...
                            <div class="modal fade" id="ml-general-stock" tabindex="-1" aria-labelledby="exampleModalLabel"
                                ...
                                <div class="modal-dialog modal-lg">
                                    <form action="{{ route('ml_general_stock.store') }}" method="POST" autocomplete="off">
                                        @csrf
                                        <div class="modal-content">
                                            <div class="modal-header">
                                                <h5 class="modal-title" id="exampleModalLabel">Add Micro Lab General Stock
                                                </h5>
                ...
                <div class="table-responsive">
                    <table class="datatable table table-stripped table-hover">
                        ...
                        <tbody>
                            @foreach ($ml_general_stocks as $ml_general_stock)
                                <tr>
                                    <td>{{ $loop->index + 1 }}</td>
                                    <td>{{ $ml_general_stock->item_name }}</td>
                                    <td>{{ $ml_general_stock->description }}</td>
                                    <td>{{ $ml_general_stock->total_stock }}</td>
                                    <td>{{ $ml_general_stock->remark }}</td>
                                    <td class="d-flex justify-content-start">
                                        <a href="{{ url('ml_general_stock/purchase', $ml_general_stock->id) }}">
                                            <button type="button" class="btn btn-md btn-primary py-2 px-1"><i
                                                    class="fas fa-plus-circle pr-0"></i><b>Purchase</b></button>
                                        </a>
                                        <a href="{{ url('ml_general_stock/used', $ml_general_stock->id) }}">
                                            <button type="button" class="btn btn-md btn-primary py-2 px-1"><i
                                                    class="fas fa-minus-circle m-0"></i>
                                                <b>Used</b></button>
                                        </a>
                                        <a href="{{ route('ml_general_stock.show', $ml_general_stock->id) }}"><button
                                                type="button" class="btn btn-md btn-info py-2 px-1"><i
                                                    class="fas fa-eye m-0"></i>
                                                <b>Detail</b></button></a>
                                        <a href="{{ route('ml_general_stock.edit', $ml_general_stock->id) }}"><button
                                                type="button" class="btn btn-md btn-warning py-2 px-1"><i
                                                    class="fas fa-edit m-0"></i>
                                                Edit</button></a>

                                        <form action="{{ route('ml_general_stock.destroy', $ml_general_stock->id) }}"
                                            method="post">
                                            {{ csrf_field() }}
                                            {{ method_field('DELETE') }}
                                            <button type="submit" class="btn btn-md btn-danger py-2 delete"
                                                id="{{ $ml_general_stock->id }}"><i class="fas fa-trash"></i>
                                                Delete</button>
                                        </form>
                                    </td>
                                </tr>
