        ...
        <div class="row">
            <div class="col-sm-12">
                <div class="card mb-3 card-button">
                    <div class="card-header d-flex justify-content-between">
                        <h4 class="mt-2"><b>Used FIDSL General Stock List</small></b></h4>
            ...
            <div class="col-sm-6">
                <div class="card bg-primary text-light">
                    ...
                    <form action="{{ route('fidsl_general_stock_used.store') }}" method="POST" autocomplete="off">
                        @csrf
                        ...
                        <tbody>
                            @foreach ($p_f_general_stocks as $p_f_general_stock)
                                <tr>
                                    ...
                                    <td>{{ date('d-M-Y', strtotime($p_f_general_stock->used_date)) }}</td>
                                    <td>{{ $p_f_general_stock->used_stock }}</td>
                                    {{-- <td>

                                        <form
                                            action="{{ route('fidsl_general_stock_used.destroy', $p_f_general_stock->id) }}"
                                            method="post">
                                            {{ csrf_field() }}
                                            {{ method_field('DELETE') }}
...
@section('script')
    <script>
        ...
        //             $.ajax({
        //                 type: 'DELETE',
        //                 url: '/fidsl_general_stock_used/' + id,
        //                 success: function() {
        //                     Swal.fire({
        //                         icon: 'success',
