            ...
            <div class="col-sm-12">
                <div class="card mb-3 card-button">
                    <div class="card-header d-flex justify-content-between">
                        <h4 class="mt-2"><b>FIDSL-Ad-01-01</b></h4>
                        <a href="{{ route('aw-ad0101.create') }}">
                        ...
                        <tbody>
                            @foreach ($ads as $ad)
                                <tr>
                                    ...
                                    <td>{{ $ad->contact_person }}</td>
                                    <td>{{ $ad->sample_name_code }}</td>
                                    <td class="d-flex justify-content-start">
                                        <a href="{{ route('aw-ad0101.show', $ad->id) }}"><button type="button"
                                                class="btn btn-sm btn-primary py-2"><i class="fas fa-eye"></i>
                                                Detail</button></a>
                                        <a href="{{ route('aw-ad0101.print', $ad->id) }}"><button type="button"
                                                class="btn btn-sm btn-success py-2"><i class="fas fa-print"></i>
                                                Report</button></a>
                                        <a href="{{ route('aw-ad0101.edit', $ad->id) }}"><button type="button"
                                                class="btn btn-sm btn-warning py-2"><i class="fas fa-edit"></i>
                                                Edit</button></a>

                                        <form action="{{ route('aw-ad0101.destroy', $ad->id) }}" method="post">
                                            {{ csrf_field() }}
                                            {{ method_field('DELETE') }}
