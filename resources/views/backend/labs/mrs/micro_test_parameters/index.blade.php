        ...
        <div class="col-sm-12">
            <div class="card mb-3 card-button">
                <div class="card-header d-flex justify-content-between">
                    <h4 class="mt-2"><b>Micro Test Parameter</small></b></h4>
            ...
            <div class="table-responsive">
                <table class="datatable table table-stripped table-hover">
                    ...
                    <tbody>
                        @foreach ($micro_tests as $test)
                            <tr>
                                <td>{{ $loop->index + 1 }}</td>
                                <td>{{ $test->name }}</td>
                                <td>
                                    <form action="{{ route('micro_test_parameters.update', $test->id) }}" method="POST"
                                        autocomplete="off">
                                        @csrf
                                        {{ method_field('PUT') }}
                                ...
                                <td>
                                    <form action="{{ route('micro_test_parameters.destroy', $test->id) }}" method="post">
                                        {{ csrf_field() }}
                                        {{ method_field('DELETE') }}
