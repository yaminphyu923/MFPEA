                            ...
                            @foreach ($fcars as $car)
                                <tr>
                                    ...
                                    <td class="d-flex justify-content-start">
                                        <a href="{{ route('fcar0502.show', $car->id) }}"><button type="button"
                                                class="btn btn-sm btn-primary py-2"><i class="fas fa-eye"></i>
                                                Detail</button></a>
                                        <a href="{{ route('fcar0502.print', $car->id) }}"><button type="button"
                                                class="btn btn-sm btn-success py-2"><i class="fas fa-print"></i>
                                                Print</button></a>
                                        <a href="{{ route('fcar0502.edit', $car->id) }}"><button type="button"
                                                class="btn btn-sm btn-warning py-2"><i class="fas fa-edit"></i>
                                                Edit</button></a>
                                        <form action="{{ route('fcar0502.destroy', $car->id) }}" method="post">
                                            {{ csrf_field() }}
                                            {{ method_field('DELETE') }}
