                ...
                <div class="card mb-3 card-button">
                    <div class="card-header d-flex justify-content-between">
                        ...
                        {{-- <a href="{{ route('other_tests.index') }}">
                            <button type="button" class="btn btn-md btn-primary"><b>other Test
                                    Parameter</b></button>
                        </a> --}}
                        ...
                        <tbody>
                            @foreach ($others as $other)
                                <tr>
                                    ...
                                    <td>{{ date('d-M-Y', strtotime($other->lab_received_date)) }}</td>
                                    <td>{{ $other->product_name }}</td>
                                    <td>{{ $other->phone }}</td>
                                    <td class="d-flex justify-content-start">
                                        <a href="{{ route('others.show', $other->id) }}"><button type="button"
                                                class="btn btn-sm btn-primary py-2"><i class="fas fa-eye"></i>
                                                Detail</button></a>
                                        <a href="{{ route('others.print', $other->id) }}"><button type="button"
                                                class="btn btn-sm btn-success py-2"><i class="fas fa-print"></i>
                                                Report</button></a>
                                        <a href="{{ route('others.edit', $other->id) }}"><button type="button"
                                        ...
                                        <form action="{{ route('others.destroy', $other->id) }}" method="post">
                                            ...
                                            <button type="submit" class="btn btn-sm btn-danger py-2 delete"
                                                id="{{ $other->id }}"><i class="fas fa-trash"></i> Delete</button>
                                        </form>
                                    </td>
                                </tr>
                            @endforeach
