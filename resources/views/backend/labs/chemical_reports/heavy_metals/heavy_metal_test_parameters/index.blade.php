        ...
        <div class="col-sm-12">
            <div class="card mb-3 card-button">
                <div class="card-header d-flex justify-content-between">
                    <h4 class="mt-2"><b>Heavy Metal and Essential Report Test Parameter</small></b></h4>
                    <div class="justify-content-end">
                        ...
                        <div class="modal fade" id="member" tabindex="-1" aria-labelledby="exampleModalLabel"
                            ...
                            <div class="modal-dialog">
                                <form action="{{ route('heavy_metal_tests.store') }}" method="POST">
                                    @csrf
            ...
            <div class="table-responsive">
                <table class="datatable table table-stripped table-hover">
                    ...
                    <tbody>
                        @foreach ($heavy_metal_tests as $test)
                            <tr>
                                <td>{{ $loop->index + 1 }}</td>
                                <td>{{ $test->name }}</td>
                                <td>
                                    <form action="{{ route('heavy_metal_tests.update', $test->id) }}" method="POST"
                                        autocomplete="off">
                                        @csrf
                                        {{ method_field('PUT') }}
                                ...
                                <td>
                                    <form action="{{ route('heavy_metal_tests.destroy', $test->id) }}" method="post">
                                        {{ csrf_field() }}
                                        {{ method_field('DELETE') }}
