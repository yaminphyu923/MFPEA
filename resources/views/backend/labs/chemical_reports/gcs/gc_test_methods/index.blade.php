        ...
        <div class="col-sm-12">
            <div class="card mb-3 card-button">
                <div class="card-header d-flex justify-content-between">
                    <h4 class="mt-2"><b>GC Test Method</small></b></h4>
                    <div class="justify-content-end">
                        ...
                        <div class="modal fade" id="member" tabindex="-1" aria-labelledby="exampleModalLabel"
                            ...
                            <div class="modal-dialog">
                                <form action="{{ route('gc_test_methods.store') }}" method="POST">
                                    @csrf
            ...
            <div class="table-responsive">
                <table class="datatable table table-stripped table-hover">
                    ...
                    <tbody>
                        @foreach ($gc_methods as $method)
                            <tr>
                                <td>{{ $loop->index + 1 }}</td>
                                <td>{{ $method->name }}</td>
                                <td>
                                    <form action="{{ route('gc_test_methods.update', $method->id) }}" method="POST"
                                        autocomplete="off">
                                        @csrf
                                        {{ method_field('PUT') }}
                                        <div class="form-group row">
                                            <div class="col-sm-6">
                                                <input type="text" name="name" id="name"
                                                    value="{{ $method->name }}" class="form-control">
                                            </div>
                                ...
                                <td>
                                    <form action="{{ route('gc_test_methods.destroy', $method->id) }}" method="post">
                                        {{ csrf_field() }}
                                        {{ method_field('DELETE') }}
