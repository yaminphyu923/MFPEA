            ...
            <div class="col-sm-12">
                <div class="card mb-3 card-button">
                    <div class="card-header d-flex justify-content-between">
                        <h4 class="mt-2"><b>F-M01-01-01</b></h4>
                        <a href="{{ route('m010101.create') }}">
                            ...
                            @foreach ($ms as $m)
                                <tr>
                                    ...
                                    <td class="d-flex justify-content-start">
                                        <a href="{{ route('m010101.show', $m->id) }}"><button type="button"
                                                class="btn btn-sm btn-primary py-2"><i class="fas fa-eye"></i>
                                                Detail</button></a>
                                        <a href="{{ route('m010101.print', $m->id) }}"><button type="button"
                                                class="btn btn-sm btn-success py-2"><i class="fas fa-print"></i>
                                                Print</button></a>
                                        <a href="{{ route('m010101.edit', $m->id) }}"><button type="button"
                                                class="btn btn-sm btn-warning py-2"><i class="fas fa-edit"></i>
                                                Edit</button></a>
                                        <form action="{{ route('m010101.destroy', $m->id) }}" method="post">
                                            {{ csrf_field() }}
                                            {{ method_field('DELETE') }}
