            ...
            <div class="col-sm-12">
                <div class="card mb-3 card-button">
                    <div class="card-header d-flex justify-content-between">
                        <h4 class="mt-2"><b>F-Car-05-01(Test Parameter)</b></h4>
                        <div class="d-flex justify-content-end">
                            <a href="{{ route('car0501.create') }}">
                            ...
                            <button type="button" class="btn btn-md btn-primary" data-toggle="modal"
                                data-target="#car0501_test"><i class="fas fa-plus-circle"></i>
                                <b>Create</b></button>
                        </div>
                    </div>
                </div>

                <div class="modal fade" id="car0501_test" tabindex="-1" role="dialog"
                    aria-labelledby="exampleModalLabel" aria-hidden="true">
                    <div class="modal-dialog" role="document">
                        <div class="modal-content">
                            <form action="{{ route('car0501-test.store') }}" method="POST">
                                @csrf
                ...
                <div class="table-responsive">
                    <table class="datatable table table-stripped table-hover">
                        ...
                        <tbody>
                            @foreach ($car0501_tests as $car0501_test)
                                <tr>
                                    <td>{{ $loop->index + 1 }}</td>
                                    <td>{{ $car0501_test->test_parameter }}</td>
                                    <td>
                                        <a href="{{ route('car0501-test.edit', $car0501_test->id) }}"><button
                                                type="button" class="btn btn-sm btn-warning py-2"><i
                                                    class="fas fa-edit"></i>
                                                Edit</button></a>
                                        <button type="button" class="btn btn-sm btn-danger py-2 delete"
                                            id="{{ $car0501_test->id }}"><i class="fas fa-trash"></i>Delete</button>
                                    </td>
                                </tr>
                            @endforeach
                        </tbody>
